...
def calculate_features(df_raw):
    ...
    df['vwap_5'] = df['mock_volume'] * df['Close'] / (df['mock_volume'].rolling(5).sum() + 1e-9)
...
def run_live_simulator():
    ...
    ai_model = CausalMultiTimeframeAI(input_dim=len(feature_cols), d_model=256, nhead=8, num_layers=3)
    ...
    if pos_manager.get('position') != 0 and entry_time_str:
        try:
            ...
            if entry_date != datetime.now().date():
                ...
                if active_contract_obj:
                    try:
                        quote_type = sj.QuoteType if hasattr(sj, 'QuoteType') else sj.constant.QuoteType
                        subscribe_func = engine.api.subscribe if hasattr(engine.api, 'subscribe') else engine.api.quote.subscribe
                        subscribe_func(active_contract_obj, quote_type=quote_type.Tick)
                        subscribe_func(active_contract_obj, quote_type=quote_type.BidAsk)
    ...
    # 嚴格過濾掉夜盤資料 (15:00 - 05:00)，確保技術指標 (如 VWAP, Daily Open) 僅以日盤計算
    df_raw = df_raw[df_raw['date'].dt.time.between(datetime_time(8, 45), datetime_time(13, 45))].reset_index(drop=True)
    
    print(f"✅ 成功載入 {len(df_raw)} 筆初始日盤 K 線！")
    ...
    def on_tick_fop_callback(tick):
        try:
            ...
            # 拒絕夜盤報價污染日盤資料結構
            if not is_market_open(now.time()): return
    ...
    def on_bidask_fop_v1_callback(bidask):
        try:
    ...
    try:
        # 兼容不同版本的 Shioaji API
        if hasattr(engine.api, 'set_on_tick_fop_v1_callback'):
            engine.api.set_on_tick_fop_v1_callback(on_tick_fop_callback)
        elif hasattr(engine.api.quote, 'set_on_tick_fop_v1_callback'):
            engine.api.quote.set_on_tick_fop_v1_callback(on_tick_fop_callback)

        if hasattr(engine.api, 'set_on_bidask_fop_v1_callback'):
            engine.api.set_on_bidask_fop_v1_callback(on_bidask_fop_v1_callback)
        elif hasattr(engine.api.quote, 'set_on_bidask_fop_v1_callback'):
            engine.api.quote.set_on_bidask_fop_v1_callback(on_bidask_fop_v1_callback)

        if hasattr(engine.api, 'set_on_tick_fnc'):
            engine.api.set_on_tick_fnc(quote_callback)
        elif hasattr(engine.api.quote, 'set_on_tick_fnc'):
            engine.api.quote.set_on_tick_fnc(quote_callback)
        elif hasattr(engine.api, 'set_quote_callback'):
            engine.api.set_quote_callback(quote_callback)
        ...
        quote_type = sj.QuoteType if hasattr(sj, 'QuoteType') else sj.constant.QuoteType
        quote_version = sj.QuoteVersion if hasattr(sj, 'QuoteVersion') else (sj.constant.QuoteVersion if hasattr(sj.constant, 'QuoteVersion') else None)

        subscribe_func = engine.api.subscribe if hasattr(engine.api, 'subscribe') else engine.api.quote.subscribe

        # 確保以 v1 版本訂閱 Tick 與 BidAsk
        if quote_version:
            subscribe_func(txf_contract, quote_type=quote_type.Tick, version=quote_version.v1)
            subscribe_func(txf_contract, quote_type=quote_type.BidAsk, version=quote_version.v1)
        else:
            subscribe_func(txf_contract, quote_type=quote_type.Tick)
    except Exception as e:
        ...
        try:
            ...
            if event_type == 'TXF_TICK':
                ...
                if bar:
                    ...
                    # MTF processing
                    df_1m = df[feature_cols].copy()
                    for col in ['mock_volume', 'macd_hist', 'vwap_bias']:
                        if col in df_1m.columns:
                            df_1m[col] = np.sign(df_1m[col]) * np.log1p(np.abs(df_1m[col]))
                    
                    df_norm = pd.DataFrame(np.nan_to_num((df_1m.values - mean_v) / np.where(std_v == 0, 1.0, std_v), nan=0.0), columns=feature_cols)
                    df_norm['date'] = df['date'].reset_index(drop=True)
                    
                    win_1m, win_15m = WINDOW_SIZE, 20
                    df_15m_historical = df_norm.set_index('date').resample('15min', closed='right', label='right').last().dropna()
                    
                    current_1m_state = df_norm.drop(columns='date').values[-1].reshape(1, -1)
                    hist_15m = df_15m_historical.values[-(win_15m-1):] if len(df_15m_historical) > 0 else np.empty((0, df_norm.shape[1]-1))
                    ...
                    seq_1m = df_norm.drop(columns='date').values[-win_1m:]
                    
                    # 取 device
                    device = next(ai_model.parameters()).device
                    feat_1m = torch.tensor(seq_1m, dtype=torch.float32).unsqueeze(0).to(device)
                    feat_15m = torch.tensor(seq_15m[-win_15m:], dtype=torch.float32).unsqueeze(0).to(device)

                    with torch.no_grad():
                        dir_0 = torch.zeros((1, 1), dtype=torch.float32).to(device)
                        dir_1_buy = torch.ones((1, 1), dtype=torch.float32).to(device)
                        dir_1_sell = -torch.ones((1, 1), dtype=torch.float32).to(device)

                        y0_logits, _, _ = ai_model(feat_1m, feat_15m, dir_0)
                        _, y1_logits_buy, _ = ai_model(feat_1m, feat_15m, dir_1_buy)
                        _, y1_logits_sell, _ = ai_model(feat_1m, feat_15m, dir_1_sell)

                        probs = torch.softmax(y0_logits, dim=1).squeeze().cpu().numpy()
                        y1_probs_buy = torch.softmax(y1_logits_buy, dim=1).squeeze().cpu().numpy()
                        y1_probs_sell = torch.softmax(y1_logits_sell, dim=1).squeeze().cpu().numpy()

                        classes = np.array([-3, -2, -1, 0, 1, 2, 3])
                        expected_y0 = np.sum(probs * classes)
                        expected_y1_buy = np.sum(y1_probs_buy * classes)
                        expected_y1_sell = np.sum(y1_probs_sell * classes)

                        cate_buy = expected_y1_buy - expected_y0
                        cate_sell = expected_y0 - expected_y1_sell
                    ...
                    if pos_manager.get('position') == 0 and is_market_open(now.time()) and not is_eod_closing_time(now, is_settlement=is_today_settlement):
                        ...
                        # --- Causal Inference Filtering ---
                        CATE_THRESHOLD = 0.5  # 交易成本閾值
                        if signal > 0:
                            if cate_buy < CATE_THRESHOLD:
                                print(f"[{time_str}] 🛡️ 因果推論過濾：買盤干預效應不足 (CATE_Buy={cate_buy:.2f} < {CATE_THRESHOLD})，擋下潛在假突破多單。")
                                signal = 0
                        elif signal < 0:
                            if cate_sell < CATE_THRESHOLD:
                                print(f"[{time_str}] 🛡️ 因果推論過濾：賣盤干預效應不足 (CATE_Sell={cate_sell:.2f} < {CATE_THRESHOLD})，擋下潛在假跌破空單。")
                                signal = 0
                        ...
                        if signal != 0:
                            ...
                            try:
                                quote_type = sj.QuoteType if hasattr(sj, 'QuoteType') else sj.constant.QuoteType
                                subscribe_func = engine.api.subscribe if hasattr(engine.api, 'subscribe') else engine.api.quote.subscribe
                                subscribe_func(active_contract, quote_type=quote_type.Tick)
                                subscribe_func(active_contract, quote_type=quote_type.BidAsk)
                            except:
                                pass
